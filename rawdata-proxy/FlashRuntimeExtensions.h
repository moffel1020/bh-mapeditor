...
typedef FREObject (*FREFunction)(FREContext ctx, void* functionData, uint32_t argc,
                                 FREObject argv[]);
 ...
 *
 * @param ctxType Pointer to the contextType string (UTF8) as provided to the AS
 * createExtensionContext call.
 *
...
typedef void (*FREContextInitializer)(void* extData, const uint8_t* ctxType, FREContext ctx,
...
typedef void (*FREInitializer)(void** extDataToSet, FREContextInitializer* ctxInitializerToSet,
...
typedef enum {
    ...
    FREResult_ENUMPADDING =
        0xfffff /* will ensure that C and C++ treat this enum as the same size. */
} FREResult;
...
FREResult FRENewObject(const uint8_t* className, uint32_t argc, FREObject argv[], FREObject* object,
...
FREResult FREGetObjectProperty(FREObject object, const uint8_t* propertyName,
...
FREResult FRESetObjectProperty(FREObject object, const uint8_t* propertyName,
                               FREObject propertyValue, FREObject* thrownException);
...
FREResult FRECallObjectMethod(FREObject object, const uint8_t* methodName, uint32_t argc,
                              FREObject argv[], FREObject* result, FREObject* thrownException);
...
typedef struct {
    ...
    uint32_t hasAlpha; /* if non-zero, pixel format is ARGB32, otherwise pixel format is _RGB32,
                          host endianness */
    uint32_t isPremultiplied; /* pixel color values are premultiplied with alpha if non-zero,
                                 un-multiplied if zero */
...
typedef struct {
    ...
    uint32_t isPremultiplied; /* pixel color values are premultiplied with alpha if non-zero,
                                 un-multiplied if zero */
    uint32_t lineStride32; /* line stride in number of 32 bit values, typically the same as width */
    uint32_t isInvertedY;  /* if non-zero, last row of pixels starts at bits32, otherwise, first row
                              of pixels starts at bits32. */
    uint32_t* bits32;      /* pointer to the first 32-bit pixel of the bitmap data */
} FREBitmapData2;
...
FREResult FREInvalidateBitmapDataRect(FREObject object, uint32_t x, uint32_t y, uint32_t width,
                                      uint32_t height);
/**
 * @return  FRE_OK
 *          FRE_WRONG_THREAD
...
FREResult FRENewByteArray(FREByteArray* byteArrayData, FREObject* handle);
...
typedef struct {
    int if_using_FRENativeWindow_please_include_gtk_headers_before_this_one;
} GtkWindow;
 ...
 *
 * @param objExtensionContext The ExtensionContext (ActionScript object) for which the FREContext
 * handle is required.
 *
 ...
 *          FRE_ILLEGAL_STATE
 */
FREResult FREGetFREContextFromExtensionContext(FREObject objExtensionContext, FREContext* pContext);
